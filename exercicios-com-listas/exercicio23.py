...
f = open('exercicio23_arquivo.txt','r')
usuarios = [usuario.strip() for usuario in f.readlines()]
f.close()

total_kb = 0

for usuario in usuarios:
    total_kb += int(usuario[15:])
...
for usuario in usuarios:
    nome = usuario[:15]
    tamanho = usuario[15:]
    tamanho_mb = (int(tamanho) / 1024) / 1024 # byte > kilobyte > megabyte

    print(f"{numero} {nome} {tamanho_mb:.2f}MB\t{(tamanho_mb / total_mb) * 100:.2f}%")

    numero +=1


print(f"Espaço total ocupado {total_mb:.2f}")
print(f"Espaço médio ocupado {total_mb/ len(usuarios):.2f}Mb")